	...
	{#if isShow}
		<div class="absolute z-20 min-h-screen min-w-full bg-ro-brown-light text-ro-white">
			...
			<div class="container min-h-screen min-w-full pt-80">
				...
				<div
					class="text-left leading-relaxed font-mirage text-5xl"
					in:fade={{ duration: 2000, delay: 2000 }}
				>
					Join us at Our <span class="font-island text-6xl mr-4">Special </span> Day
				</div>
				<div class="font-mirage text-xl mt-4" in:fade={{ duration: 2000, delay: 2500 }}>
					Find out about the locations, accomodations and The RSVP
				</div>
			</div>
		</div>
		<!-- Action Container -->
		<div class="container absolute bottom-8 left-0 z-20" in:fade={{ duration: 1000, delay: 3000 }}>
